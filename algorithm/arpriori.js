  ...
  (function(Apriori) {
    ...
    var Algorithm = (function() {
      ...
      Algorithm.prototype.analyze = function(transactions) {
        ...
        if (self.debugMode) {
          console.log(
            "Before finding item sets: " + self.getTime(beforeMillis) + " ms"
          );
        }
        ...
        while (currentLItemSets.length !== 0) {
          ...
          var joinedSets = ArrayUtils.toFixedSizeJoinedSets(
            currentLItemSets.map(extractItemSet),
            itemSetSize + 1
          );
          currentLItemSets = self.findItemSetsMinSupportSatisfied(
            joinedSets,
            transactions,
            self.minSupport,
            frequencies
        ...
        if (self.debugMode) {
          console.log(
            "After finding item sets: " + self.getTime(beforeMillis) + " ms"
          );
        }
        ...
        var isTheRuleAlreadyFound = function(itemSet) {
          ...
          foundSubSets.forEach(function(subset) {
            if (!found) found = subset.toString() === itemSet.toString();
          });
          return found;
        };
        ...
        if (self.debugMode) {
          console.log(
            "Before calculating association rules: " +
              self.getTime(beforeMillis) +
              " ms"
          );
        }
        ...
        var saveAssociationRuleIfFound = function(subsetItemSet) {
          var diffItemSet = ArrayUtils.getDiffArray(
            currentItemSet,
            subsetItemSet
          );
          if (diffItemSet.length > 0) {
            var itemSupport = calculateSupport(
                currentItemSet,
                frequencies,
                transactions
              ),
              subsetSupport = calculateSupport(
                subsetItemSet,
                frequencies,
                transactions
              ),
        ...
        var saveAllAssociationRulesIfFound = function(itemSet) {
          ...
          ArrayUtils.toAllSubSets(currentItemSet).forEach(
            saveAssociationRuleIfFound
          );
        };
        ...
        if (self.debugMode) {
          console.log(
            "After calculating association rules: " +
              self.getTime(beforeMillis) +
              " ms"
          );
        }
        ...
        if (self.debugMode) {
          ...
          console.log(
            "Apriori.Algorithm's total spent time: " +
              self.getTime(beforeMillis) +
              " ms"
          );
        }
      ...
      Algorithm.prototype.findItemSetsMinSupportSatisfied = function(
        itemSets,
        transactions,
        minSupport,
        frequencies
    ...
    var ArrayUtils = (function() {
        ...
        var op = function(n, sourceArray, currentArray, allSubSets) {
          if (n === 0) {
            if (currentArray.length > 0) {
              allSubSets[allSubSets.length] = ArrayUtils.toStringSet(
                currentArray
              );
            }
          } else {
      ...
      ArrayUtils.getDiffArray = function(arrayA, arrayB) {
        ...
        arrayA.forEach(function(e) {
          if (arrayB.indexOf(e) === -1) diffArray.push(e);
        });
        return diffArray;
      };
      ArrayUtils.readCSVToArray = function(inputString, delimiter) {
        ...
        while (!!(matched = regexp.exec(inputString))) {
          ...
          var matchedValue = matched[2]
            ? matched[2].replace(new RegExp('""', "g"), '"')
            : matched[3];
